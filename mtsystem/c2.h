...
public: 
   ...
   std::map<int,std::vector<std::string>> correct_act_words;
   ...
   //dictionaries are similar to wordsToInt with a lot of the functionality automatically included
   //this is over the output data
   cnn::Dict out_d;
...
inline void load_correct_actions(std::string file){
  ...
  while (getline(actionsFile, lineS)){
    ...
    ReplaceStringInPlace(lineS, "-LRB-", "_LRB_");
      ...
      if ((lineS[0] == 'G' || lineS[0] == 'O')) {
        int len_char = lineS.size() - 5;  //3 for gen or out, then 2 for (), so 5 total
        out_d.Convert(lineS.substr(4, len_char));//starts at the 4th position, is
        std::vector<std::string> a = correct_act_words[sentence];
                                 a.push_back(lineS.substr(4, len_char));
                                 correct_act_words[sentence] = a;
      }
      else {
        std::vector<std::string> a = correct_act_words[sentence];
                                 a.push_back(" ");
      ...
      //this adds words to the dictionary from gen and from out
			for (auto a: actions) {
				if (a==lineS || a==lineS.substr(0,3)) {
			...
			if (!found) {
        //if the operation is GEN or OUT, set the word in the dictionary to the proper word
        if (lineS[0] == 'G' || lineS[0] == 'O') {
          actions.push_back(lineS.substr(0,3));
        }
        else{
