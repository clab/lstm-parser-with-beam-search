...
def out_element(stack, out, element):
    stack.pop()
    out.append(element)
...
def rearrange(source, tar, reals, realt):
    inp = source[:]
    target = tar[:]
...
def reorder(sequence, actions):
    ...
    for action in actions:
        ...
        if action == 'SHIFT':
            shift(buf, stack)
        if action[:3] == 'OUT':
            out_element(stack, out, action[3:])
        if action == 'COPY':
            copy(buf)
        if action[:3] == 'GEN':
            oute(out, action[3:])
...
#reali = 'I went to the movies tonight'.split()
#realt = 'jinwan wo qu  dianyingyuan'.split()
...
#x = rearrange(inpu, target, reali, realt)
